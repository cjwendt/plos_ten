...
A surfacing central axiom in academic research is computational
...
long and complex---data and code must be shared to allow others to fully
understand and replicate many studies. While standards for disseminating
and communicating computational science are evolving with digitization,
responsible researchers are adopting best practices so others can more
easily interpret, validate, and extend their published knowledge
{[}4{]}. These researchers rely on accessible and robust tools to ensure
their computations are reproducible. By meeting the standards of
computational reproducibility, researchers also benefit in other
ways---they can reduce variability in scientific outcomes that results
from differences in workflows and, today, the scientific community deems
the absence of automation to be irresponsible {[}5{]} and can improve
productivity and collaboration.

A key tool in meeting the standards of computational reproducibility is
a scriptable programming language, and the R programming language is a
...
when compared to similar languages, particularly in academia,
healthcare, and government {[}6{]}. In particular, R is increasingly
popular in computational biology and bioinformatics, two of many
disciplines generating extensive, heterogeneous, and complex data
wanting for heavy-duty data analysis tools that (ideally) support
reproducibility {[}7,8{]}.

R was developed by statisticians, and its base software is
collaboratively maintained by an international group of core
contributors {[}9{]}. The language also has an ecosystem of extensions
beyond this base software---unlike several popular proprietary languages
such as MATLAB or SAS, an R user can access, alter, and extend the base
R software, and then share their work with other users in a formalized
way through extensions that are commonly called \emph{packages} (as a
note, ``package'' is not perfectly synonymous with ``extension'', as
some packages also come with base R and so are not extending the base
software, but we will use ``package'' as throughout as the more commonly
recognized colloquial term). An R package is a fundamental unit of
shareable code. There are numerous analogies in computing between
programming and culinary arts: recipe structures, coding cookbooks, and
the like. To conceptualize packages, imagine you are the chef, base R is
the kitchen, and packages are the special gadgets which allow you to
cook and bake new recipes. In essence, R packages are coding delectables
that enable the user to perform practical tasks and solve problems with
...
statistical methods, visualization, and data analysis is wrapped up in R
packages---written and documented by R users from around the world. Are
there R packages for wrangling and cleaning data frames, designing
interactive applications for data visualization, or performing
dimensionality reduction? Yes! But how do you \emph{find} an R package
to help you train regression and classification models, assess the beta
...
of the language may struggle to find packages because they do not know
where to search, what to look for, or how to sift through options. When
first learning R, two key obstacles are figuring out how to (1) find a
package to accomplish a particular task or solve a problem of interest
and (2) choose the best package to perform that task.

However, new users don't need to struggle alone in figuring this out.
...
for finding and selecting R packages, so that you will spend less time
...
great to turn to when a task has a broad or complex scope beyond what
you can (or want to) attempt from scratch. While there are ways to cook
up an algorithm in base R, a relevant package may accomplish your goal
with less code and fewer bugs. Extensive tasks justify sophisticated
...
{[}16{]}. When you look for a package to visualize the data, you will
want to choose one that gracefully handles this type of object as an
input. If you don't quite know (or have a difficult time articulating)
what you need, don't hesitate to search the internet for examples that
capture your purpose. It can be helpful to search with Google (and with
Google Images for plots and figures) to identify the terminology that
describes what you are trying to do, how people approach similar tasks,
...
directly from the R console with \texttt{ctv::CRAN.views()} {[}17{]}
...
due to the sheer quantity of available packages {[}18{]}.
...
both public and private use, but there are alternatives. A R package
repository is essentially a warehouse for tools before they are shipped
to your kitchen; in computing terms, a repository provides a central
online location in which packages are stored and managed. Some
repositories impose vetting mechanisms that tame unwieldy aspects of the
R ecosystem by regularly checking underlying code and managing
corresponding webs of dependencies. Two traditional repositories for R
packages are CRAN and Bioconductor; however, there are also lesser-known
repositories to explore. Developers can also share their R packages
...
format or content of shared code compared to repositories. For more
private sharing, rather than making packages accessible to everyone via
...
multidimensional maps {[}9,22,23{]}. Due to its longevity and historical
role, Rickert asserts that ``CRAN is the greatest open source repository
for statistical computing knowledge in the world'' {[}21{]}. A key
advantage of using a package from CRAN is that the repository integrates
easily with your base R installation {[}24{]}. You can simply use the
\texttt{install.packages()} function to install a package from CRAN,
along with all of the packages it depends on; source and/or binary code
are automatically saved to your computer in a designated package library
{[}25,26{]}. When you want to use a particular package, you load it to
your R session via the \texttt{library()} function from base R {[}26{]}.
These features ensure that integration of a new package with your
version of the base R software is typically silent and seamless.
...
{[}27{]}. CRAN does not, however, have a peer-review process (see Rule 3
...
repository than CRAN that's worth investigating {[}28{]}. The
...
{[}28{]}. Bioconductor boasts a modularized design, wherein data
...
experience with R (see Rule 4) {[}7{]}. Bioconductor packages also
...
packages with their corresponding dependencies {[}29{]}.
...
your current version of R {[}28{]}. This helps, among other things, keep
...
long-term, and comply with additional package guidelines {[}30{]}.
...
peer-reviewed. The peer-review processes includes a technically-focused
review by a primary reviewer and may also include secondary reviews by
content-matter experts {[}28{]}.
...
laboratory reproducibility tools {[}32{]}. R package maintainers can
also create and host their own personalized repository using the
\texttt{drat} (Drat R Archive Template) package, which enables
...
to hosting a package on these online platforms {[}21,37{]}. First,
...
data and code to ensure a stable end-product. Finally, they can be used
to share packages that don't fit under some of the policies for CRAN or
Bioconductor---for example, package that are over CRAN's recommended
size limit.
...
packages are published on CRAN or Bioconductor {[}38{]}, GitHub can
...
complication {[}21,37,40{]}. Further, when a package is installed from
...
CRAN (or Bioconductor) and many times packages become available in both
places.
...
There's been a call for the development of centralized resources in
...
of today's information society, it's questionable whether they will
...
for how to use the function in practice. If you'd like to check all help
documentation for a package, you can access an index of help pages from
the console via \texttt{help(package\ =\ "...")} {[}26{]}. The
...
leave you wanting, the package likely doesn't have further (quality)
...
of interest via the \texttt{vignette()} function from base R {[}26{]}.
...
why you should use it, how to use it, and how to install it {[}25{]}.
...
and GitHub {[}26,49{]}. RDocumentation may include: an overview,
...
\texttt{bookdown}, a relatively new extension of R Markdown that's
...
open, interactive, and downloadable online book {[}50{]}. The
\texttt{bookdown} package itself has an
\href{https://bookdown.org/yihui/bookdown/}{online book} that details
usage of the package {[}50,51{]}.

Other help documentation that's shared outside of the package's code
...
Models in R using the \texttt{mgcv} package {[}52{]}. Every now and
...
organized displays of outputs from these packages {[}54--56{]}. The
...
the \texttt{Rcpp} package {[}57{]}. RStudio hosts cheatsheets of concise
...
(survival plots) {[}11,39,42,54,58--63{]}. Further information about
...
documentation. In this case, you'll use resources that involve
\emph{asking} for help---should the occasion arise, you want to be
assured that you'll find a satisfactory answer. The R-help mailing list,
to which you can subscribe and send questions, is moderated by the R
...
{[}64{]}. In the early days, the R-help mailing list was the only way to
seek direct assistance; since, the R community (e.g.,
\href{https://community.rstudio.com/}{RStudio Community},
\href{https://support.bioconductor.org/}{Bioconductor Support}) has
evolved to make asking for help more widespread and accessible, with
inclusion and creative problem-solving as hallmarks of its online
presence {[}65{]}. Certain packages have independent mailing lists;
\texttt{statnet} is an example of a suite of packages that has its own
\href{http://statnet.org/}{community mailing list} {[}66{]}. If a
...
language by these metrics {[}6,67{]}. When you encounter a problem, it's
...
finding or posting a reproducible example {[}68{]}.

One example of a package---mentioned a few times already---with notable
documentation and first-rate help resources is \texttt{Rcpp} {[}57{]}.
...
described in a print book by its maintainer {[}69{]} and in chapters or
sections of other books (e.g., {[}68{]}).
...
popular a tool is and whether it's stood the test of time. Since there
...
updates on a repository's activity {[}70{]}. Although not a definitively
...
{[}71{]}. If a package is posted on CRAN or Bioconductor, its page on
...
calculation {[}72{]}. You can research stats on corresponding
...
A Package for Helping You Choose Which Package to Use} {[}73{]}. Fun
fact: the \texttt{packagemetrics} project was born at an Unconference
{[}74{]}. You can install \texttt{packagemetrics} via
...
the input is a vector of packages of interest, by name {[}39,73{]}.
...
you just described and the resulting output.{]}}) {[}73,75{]}.
...
packages (see Rule 3) {[}31{]}. The rOpenSci organization oversees peer
review of R packages at a highly detailed level that's increasingly
...
manage the data lifecycle {[}32{]}. Their review process is conducted
...
print the suggested BibTex citation for the package {[}26{]}. Further,
...
{[}76{]}. You can search for packages directly by name in Google
...
package. While it's best practice to cite R packages using these
...
is grounded in statistical computing and graphics, there's variation in
...
levels of involvement. This Rule doesn't imply that worthwhile packages
...
decisions. In fact, as you become more immersed in the R community,
you'll find that name recognition is a factor, among many, that helps
you establish trust in certain tools more quickly than others {[}71{]}.
...
package to access analogous package-specific metadata {[}54{]}. Here,
...
altering the web address as previously described {[}77{]}.
...
current one {[}54{]}. Oftentimes, existence of a large team can give you
...
release {[}54{]}. This might reveal that a package is part of a
...
fixes, general improvements, new functionality) {[}1,25{]}. You can
...
an automated process called unit testing {[}25,81{]}. This helps assure
...
visualize progress through automatic code testing {[}82,83{]}.
...
and package levels {[}84{]}. You can look at the source code for a
...
{[}85{]}. They're easily visible, interactive, and increase readability
...
passing continuous integration tests, which you'll learn more about soon
{[}70{]}. Badges are also clickable buttons that link to a page that
describes and displays detailed aspects of the metric associated with
the badge. For instance, when you click on a \texttt{codecov} badge,
you'll likely end up on a page with a timeline, graphics, and
...
package development and computational literacy more broadly {[}25,86{]}.
...
about lab procedures are to experimental science'' {[}{[}86{]}; p6{]}.
...
{[}87{]}. GitHub works in conjunction with Git to provide a powerful
...
{[}1,88{]}. As you may expect, Git coupled with GitHub is the version
...
everything. Just as you're happy to know that a manufacturer guarantees
...
a ``pipeline'', expects dataframe objects {[}75{]}. Because of this,
...
structure of the \texttt{tidyverse} workflow {[}75,90,91{]}.
...
slight modifications after each iteration {[}68{]}. Since R packages are
...
mind {[}92{]}. Packages need not be formal nor entirely cohesive. For
instance, personal R packages such as \texttt{Hmisc} and
\texttt{broman}, are comprised of miscellaneous functions which the
creator has developed and frequently uses {[}93,94{]}. They're like the
...
{[}54{]}. In this case, you might want to write some of your own
...
motivation behind the widely used \texttt{broom} package {[}90,95{]}.
...
and contained in a personal R package {[}54,89{]}.
...
{[}33,39,83,96--98{]}---while these packages can provide
...
18. Zeileis A. CRAN task views. R News. 2005;5: 39--40.

\leavevmode\hypertarget{ref-smith2017}{}%
19. Smith D. The r community is one of r's best features {[}Internet{]}.
...
27. CRAN repository policy {[}Internet{]}. The R Foundation; 2020.
Available:
\url{https://cran.r-project.org/web/packages/policies.html\#Submission}

\leavevmode\hypertarget{ref-bioconductor}{}%
28. Bioconductor: Open source software for bioinformatics
...
54. Wickham H. Ggplot2: Elegant graphics for data analysis
{[}Internet{]}. Springer-Verlag New York; 2016. Available:
\url{https://ggplot2.tidyverse.org}

\leavevmode\hypertarget{ref-shiny}{}%
55. Chang W, Cheng J, Allaire J, Xie Y, McPherson J. Shiny: Web
application framework for r {[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=shiny}

\leavevmode\hypertarget{ref-htmlwidgets}{}%
56. Vaidyanathan R, Xie Y, Allaire J, Cheng J, Russell K. Htmlwidgets:
...
57. Eddelbuettel D, François R. Rcpp: Seamless R and C++ integration.
...
58. Kuhn M. Caret: Classification and regression training
{[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=caret}

\leavevmode\hypertarget{ref-datatable}{}%
59. Dowle M, Srinivasan A. Data.table: Extension of `data.frame`
{[}Internet{]}. 2019. Available:
\url{https://CRAN.R-project.org/package=data.table}

\leavevmode\hypertarget{ref-leaflet}{}%
60. Cheng J, Karambelkar B, Xie Y. Leaflet: Create interactive web maps
...
61. Coppock A. Randomizr: Easy-to-use tools for common forms of random
assignment and sampling {[}Internet{]}. 2019. Available:
\url{https://CRAN.R-project.org/package=randomizr}

\leavevmode\hypertarget{ref-stringr}{}%
62. Wickham H. Stringr: Simple, consistent wrappers for common string
operations {[}Internet{]}. 2019. Available:
\url{https://CRAN.R-project.org/package=stringr}

\leavevmode\hypertarget{ref-survminer}{}%
63. Kassambara A, Kosinski M, Biecek P. Survminer: Drawing survival
...
64. Team RC. Mailing lists {[}Internet{]}. The R Foundation; 2020.
Available: \url{https://www.r-project.org/mail.html}

\leavevmode\hypertarget{ref-chase2020}{}%
65. Chase W. Dataviz and the 20th anniversary of r, an interview with
...
66. Handcock MS, Hunter DR, Butts CT, Goodreau SM, Krivitsky PN, Morris
...
67. Muenchen RA. The popularity of data analysis software. URL
http://r4statscom/popularity. 2012;

\leavevmode\hypertarget{ref-wickham2019}{}%
68. Wickham H. Advanced r. CRC press; 2019.

\leavevmode\hypertarget{ref-eddelbuettel2013}{}%
69. Eddelbuettel D. Seamless r and c++ integration with rcpp. Springer;
2013.

\leavevmode\hypertarget{ref-githubdocs}{}%
70. GitHub docs {[}Internet{]}. GitHub, Inc. 2020. Available:
\url{https://docs.github.com/en}

\leavevmode\hypertarget{ref-leek2015}{}%
71. Leek J. How i decide when to trust an r package {[}Internet{]}.
2015. Available:
...
72. Vannoorenberghe L. RDocumentation: Scoring and ranking
...
73. Firke S, Krouse B, Grand E, Shepherd L, Ampeh W, Frick H.
...
78. Ellis B, Haaland P, Hahne F, Le Meur N, Gopalakrishnan N, Spidlen J,
et al. FlowCore: FlowCore: Basic structures for flow cytometry data.
2019.

\leavevmode\hypertarget{ref-hahne2009}{}%
79. Hahne F, LeMeur N, Brinkman RR, Ellis B, Haaland P, Sarkar D, et al.
...
80. Taschuk M, Wilson G. Ten simple rules for making research software
more robust. PLoS computational biology. Public Library of Science;
2017;13.

\leavevmode\hypertarget{ref-hester2020}{}%
81. Hester J. How does covr work anyway? {[}Internet{]}. The R
...
84. Hester J. Covr: Test coverage for packages {[}Internet{]}. 2020.
Available: \url{https://CRAN.R-project.org/package=covr}

\leavevmode\hypertarget{ref-barts2018}{}%
85. Barts C. How to use github badges to stop feeling like a noob
...
86. Wilson GV. Where's the real bottleneck in scientific computing?
American Scientist. 2006;94: 5.

\leavevmode\hypertarget{ref-bryan2018}{}%
87. Bryan J. Excuse me, do you have a moment to talk about version
control? The American Statistician. Taylor \& Francis; 2018;72: 20--27.

\leavevmode\hypertarget{ref-perkel2016}{}%
88. Perkel J. Democratic databases: Science on github. Nature News.
2016;538: 127.

\leavevmode\hypertarget{ref-peng2017}{}%
89. Mastering software development in r {[}Internet{]}. 2017. Available:
\url{https://rdpeng.github.io/RProgDA/}

\leavevmode\hypertarget{ref-broom}{}%
90. Robinson D, Hayes A. Broom: Convert statistical analysis objects
into tidy tibbles {[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=broom}

\leavevmode\hypertarget{ref-biobroom}{}%
91. Andrew J. Bass SL David G. Robinson. Biobroom: Turn bioconductor
