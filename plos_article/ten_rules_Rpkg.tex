...
\hspace{1em}readr & Read rectangular data (e.g., csv, tsv, and fwf) & 2015 & Wickham et al. \cite{@readr} & https://readr.tidyverse.org/\\
\hspace{1em}dplyr & Grammar of tidy data manipulation & 2014 & Wickham et al. \cite{@dplyr} & https://dplyr.tidyverse.org/\\
\hspace{1em}tidyr & Tidy messy data & 2014 & Wickham \& Henry \cite{@tidyr} & https://tidyr.tidyverse.org/\\
\addlinespace[0.3em]
\multicolumn{5}{l}{\textbf{Statistical Modeling}}\\
\hspace{1em}broom\textsuperscript{*} & Tidy model output; convert statistical objects into tidy tibbles & 2014 & Robinson \& Hayes \cite{@broom} & https://broom.tidymodels.org/\\
\hspace{1em}purrr & Functional programming tools for functions and vectors & 2015 & Henry \& Wickham \cite{@purrr} & https://purrr.tidyverse.org/\\
\hspace{1em}caret & Framework for predictive modeling & 2007 & Kuhn \cite{@caert} & https://topepo.github.io/caret/index.html\\
\addlinespace[0.3em]
\multicolumn{5}{l}{\textbf{Data Visualization}}\\
\hspace{1em}ggplot2 & System for data visualization & 2007 & Wickham et al. \cite{@ggplot2} & https://ggplot2.tidyverse.org/\\
\hspace{1em}kableExtra & Build complex tables and manipulate styles & 2017 & Zhu \cite{@kableExtra} & https://haozhu233.github.io/kableExtra/\\
\hspace{1em}rmarkdown & Authoring framework for data science and reproducible research & 2014 & Allaire et al. \cite{@rmarkdown} & https://rmarkdown.rstudio.com/lesson-1.html\\
\bottomrule
\multicolumn{5}{l}{\textsuperscript{*} See the biobroom \cite{@biobroom} analog in Bioconductor}\\
\end{tabular}}
\label{table1}
\end{adjustwidth}
\end{table}
...
saved to your computer in a designated package library
{[}{\textbf{???}},24{]}. When you want to use a particular package, you
load it to your R session via the \texttt{library()} function from base
R {[}9{]}. These features ensure that integration of a new package with
your version of the base R software is typically silent and seamless.
...
\texttt{help(package\ =\ "...")} {[}{\textbf{???}}{]}. The documentation
that accompanies functions within packages is critical; the fact that
anyone can read it anytime and use it to guide their own work
...
via the \texttt{vignette()} function from base R {[}{\textbf{???}}{]}.
For a package shared through CRAN or Bioconductor, a list of the
package's vignettes can also be found on the respective homepage for the
package. Particularly for packages hosted on GitHub, developers often
...
and GitHub {[}{\textbf{???}},49{]}. RDocumentation may include: an
overview, installation instructions, examples of usage, functions,
guides, and vignettes. This is a subsidiary reason why is may be useful
to look for packages shared on these platforms (see Rule 3).
...
suggested citation for the package {[}{\textbf{???}}{]}. Further, in
...
archive so that code can be cited in academic literature {[}75{]}. You
