...
packages ({[}16,096 as of August 2020) on almost any conceivable topic,
from random forests to creating 3D maps {[}21,22{]}. Due to its
...
that the repository integrates easily with a user's base R installation.
You can simply use the \texttt{install.packages()} function to install a
package from CRAN; source and/or binary code are automatically saved to
your computer in a designated package library {[}23{]}. When you want to
use a particular package, you load it to your R session via the
\texttt{library()} function from base R. The R Foundation manages CRAN
and imposes strict regulatory practices for the selection and
maintenance of the packages they host. A package must pass a series of
stability tests in accordance with the CRAN Repository Policy before
obtaining publication privileges {[}24{]}. As such, CRAN only considers
...
If you are looking for tools specifically designed for high-throughput
genomic data, the Bioconductor repository is a more specialized
repository than CRAN that is worth investigating. The Bioconductor
...
cytometry, mass spectrometry, and image analysis. The R environment and
its package system is fundamental to the implementation of
Bioconductor's interoperable and object-oriented (S4) infrastructure.
...
roles that are accompanied by thorough documentation. Similar to CRAN,
...
long-term, exclusive to Bioconductor, and comply with additional package
guidelines {[}25{]}.
...
{[}26{]}. The repository only includes packages that have either been
...
{[}27{]}. R package maintainers can even create and host their own
personalized repository using the \texttt{drat} (Drat R Archive
Template) package {[}28{]}. In some cases, this might be done to host a
package that does not meet certain restrictions from other repositories
(e.g., to share data through a package that is larger than CRAN's size
limit for packages {[}29{]}). There are other platforms strictly for the
development, rather than distribution, of R packages such as R-Forge and
Omegahat, which are beyond the scope of this paper {[}30,31{]}.

The rapid uptick in package development and ensuing inter-repository
...
GitHub {[}20,32,33{]}. While there are practical downsides to their
...
package on GitHub {[}20,32{]}. GitHub is a popular online user interface
...
such packages are often published on CRAN or Bioconductor {[}34{]}.
...
2). It is simple to install the latest version of a package you want to
use from GitHub via \texttt{devtools::install\_github()}; however, the
decentralized nature of GitHub is not conducive to a tool that
automatically locates and installs corresponding dependencies {[}35{]}.
...
CRAN or Bioconductor. Further, \texttt{drat} enables developers to
design individual repositories and suites of coordinated repositories
for packages that are stored in and/or distributed through GitHub
{[}28,29{]}. Both R users and package developers benefit from
...
downloadable online book {[}36{]}. The \texttt{bookdown} package itself
has an \href{https://bookdown.org/yihui/bookdown/}{online book} that
details usage of the package {[}37{]}. \texttt{Rccp} is another package
...
package from the Tutorial pane in the IDE {[}38{]}. Finally, keep in
...
problem-solving as hallmarks of its online presence {[}39{]}. The modern
...
(R-packages) {[}40{]}. Certain packages have independent listservs;
...
metrics {[}6,41{]}. When you encounter a problem, it is good practice to
...
posting a reproducible example {[}42{]}. Overall, avoid using a package
...
following and widespread usage {[}43{]}. Likewise, the number of Google
...
skew the calculation {[}44{]}. You can research stats on corresponding
...
archive so that code can be cited in academic literature {[}45{]}. If a
...
source tools with peer reviewed R packages (see Rule 3) {[}26{]}.
...
establish trust in certain tools more quickly than others {[}43{]}.
...
create {[}46{]}. There are concrete ways to measure a tool's robustness
...
general improvements, new functionality) {[}1,23{]}. You can explore the
...
an automated process called unit testing {[}23,47{]}. Although
...
code that is not sensitive to major changes in the future {[}23{]}. To
...
{[}48{]}. Developers are also interested in quantifying the amount of
...
function, script, and package levels {[}49{]}. Relatedly, developers who
...
{[}50{]}. You may see, for example, license, dependency, or style
...
package development and computational literacy more broadly {[}23,51{]}.
...
about lab procedures are to experimental science'' {[}{[}51{]}; p6{]}.
...
{[}52{]}. GitHub works in conjunction with Git to provide a powerful
...
{[}1,53{]}. As you may expect, Git coupled with GitHub is the version
control duo of choice among serious R package developers {[}23{]}. Thus,
...
installation {[}46{]}. Vignettes include many common data science
...
a ``pipeline'', expects data frame objects {[}54{]}. Thus, when you are
...
\texttt{tidyverse} workflow {[}55,56{]}. Furthermore, the \texttt{caret}
...
can create them---more easily than you may think {[}23{]}. The reasons
...
{[}58,59{]}. Functions are necessary for efficiency and warranted when
you repetitiously copy and paste your code while making slight
modifications after each iteration {[}42{]}. The concept of personal R
...
documentation with reproducibility and collaboration in mind {[}60{]}.
...
of code, data, documentation, versions, and tests {[}46{]}. Similarly,
...
\texttt{roxygen2}, \texttt{rlang}, \texttt{drat}) {[}28,35,61--64{]}. In
...
28. Carl Boettiger DE with contributions by, Fultz N, Gibb S, Gillespie
...
29. Anderson GB, Eddelbuettel D. Hosting data packages via drat: A case
study with hurricane exposure data. R Journal. 2017;9.

\leavevmode\hypertarget{ref-theussl2009}{}%
30. Theußl S, Zeileis A. Collaborative software development using
...
31. Lang DT. The omegahat environment: New possibilities for statistical
...
32. McElreath R. Statistical rethinking: A bayesian course with examples
in r and stan. CRC press; 2020.

\leavevmode\hypertarget{ref-decan2016}{}%
33. Decan A, Mens T, Claes M, Grosjean P. When github meets cran: An
...
34. Decan A, Mens T, Claes M, Grosjean P. On the development and
...
35. Wickham H, Hester J, Chang W. Devtools: Tools to make developing r
packages easier {[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=devtools}

\leavevmode\hypertarget{ref-bookdown}{}%
36. Xie Y. Bookdown: Authoring books and technical documents with r
markdown {[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=bookdown}

\leavevmode\hypertarget{ref-xie2016}{}%
37. Xie Y. Bookdown: Authoring books and technical documents with r
markdown. Chapman; Hall/CRC; 2016.

\leavevmode\hypertarget{ref-ushey2020}{}%
38. Ushey K. RStudio 1.3 preview: Integrated tutorials {[}Internet{]}.
...
39. Chase W. Dataviz and the 20th anniversary of r, an interview with
...
40. Team RC. Mailing lists {[}Internet{]}. The R Foundation; 2020.
Available: \url{https://www.r-project.org/mail.html}

\leavevmode\hypertarget{ref-muenchen2012}{}%
41. Muenchen RA. The popularity of data analysis software. URL
http://r4statscom/popularity. 2012;

\leavevmode\hypertarget{ref-wickham2014}{}%
42. Wickham H. Advanced r. CRC press; 2014.

\leavevmode\hypertarget{ref-leek2015}{}%
43. Leek J. How i decide when to trust an r package {[}Internet{]}.
2015. Available:
...
44. Vannoorenberghe L. RDocumentation: Scoring and ranking
...
49. Hester J. Covr: Test coverage for packages {[}Internet{]}. 2020.
Available: \url{https://CRAN.R-project.org/package=covr}

\leavevmode\hypertarget{ref-barts2018}{}%
50. Barts C. How to use github badges to stop feeling like a noob
...
51. Wilson GV. Where's the real bottleneck in scientific computing?
American Scientist. 2006;94: 5.

\leavevmode\hypertarget{ref-bryan2018}{}%
52. Bryan J. Excuse me, do you have a moment to talk about version
control? The American Statistician. Taylor \& Francis; 2018;72: 20--27.

\leavevmode\hypertarget{ref-perkel2016}{}%
53. Perkel J. Democratic databases: Science on github. Nature News.
2016;538: 127.

\leavevmode\hypertarget{ref-tidyverse}{}%
54. Wickham H, Averick M, Bryan J, Chang W, McGowan LD, François R, et
al. Welcome to the tidyverse. Journal of Open Source Software. 2019;4:
1686.
doi:\href{https://doi.org/10.21105/joss.01686}{10.21105/joss.01686}

\leavevmode\hypertarget{ref-broom}{}%
55. Robinson D, Hayes A. Broom: Convert statistical analysis objects
into tidy tibbles {[}Internet{]}. 2020. Available:
\url{https://CRAN.R-project.org/package=broom}

\leavevmode\hypertarget{ref-biobroom}{}%
56. Andrew J. Bass SL David G. Robinson. Biobroom: Turn bioconductor
